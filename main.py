...
import parental_agent
import debug_agent
import share_automation
...
def _parse_intent(text):
    ...
    if t in {"debug watch", "watch debug"}:
        return ("debug-watch", {})
    if t.startswith("debug run ") or t.startswith("debug monitor ") or (t.startswith("debug ") and t not in {"debug report", "debugging report", "debug watch", "watch debug"}):
        raw_cmd = raw
        lower_cmd = raw.lower()
        if lower_cmd.startswith("debug run "):
            rest = raw_cmd[len("debug run "):].strip()
        elif lower_cmd.startswith("debug monitor "):
            rest = raw_cmd[len("debug monitor "):].strip()
        else:
            rest = raw_cmd[len("debug "):].strip()
        ae = False
        show_logs = True
        show_metrics = True
        llm_root = False
        llm_model = None
        # simple flags in the natural language command
        if " --ae" in lower_cmd or " with ae" in lower_cmd or " with anomaly" in lower_cmd:
            ae = True
            rest = re.sub(r"(--ae|with ae|with anomaly)", "", rest, flags=re.I).strip()
        if " with llm" in lower_cmd or " llm root" in lower_cmd or " root cause" in lower_cmd:
            llm_root = True
            rest = re.sub(r"(with llm|llm root|root cause)", "", rest, flags=re.I).strip()
        # Optional model selection: "model=qwen2.5:7b" or "model qwen2.5:7b"
        m = re.search(r"\\bmodel\\s*[:=]\\s*([^\\s]+)", rest, flags=re.I)
        if not m:
            m = re.search(r"\\bmodel\\s+([^\\s]+)", rest, flags=re.I)
        if m:
            llm_model = m.group(1).strip()
            rest = re.sub(r"\\bmodel\\s*[:=]?\\s*[^\\s]+", "", rest, flags=re.I).strip()
        if " quiet" in lower_cmd or " silent" in lower_cmd:
            show_logs = False
            show_metrics = False
            rest = re.sub(r"(quiet|silent)", "", rest, flags=re.I).strip()
        return ("debug-run", {"cmd": rest, "ae": ae, "show_logs": show_logs, "show_metrics": show_metrics, "llm_root": llm_root, "llm_model": llm_model})
    ...
    if m:
        path = m.group(1).strip()
        return ("smart-file", {"args": ["read", path]})
    m = re.match(r"(?i)^share\\s+(.+?)\\s+via\\s+(.+)$", raw)
    if m:
        path = m.group(1).strip()
        channel = m.group(2).strip()
        return ("share-file", {"path": path, "channel": channel})
...
PARENTAL_AGENT = parental_agent.ParentalControlAgent()
...
def _handle_intent(intent, payload):
    ...
    if intent == "help":
        ...
        print("Share file: share <file> via <gmail|drive|whatsapp>")
        return "ok"
    ...
    if intent == "debug-run":
        ...
        if not cmd:
            print("Usage: debug run <command>")
            return "ok"
        ae = bool(payload.get("ae"))
        llm_root = bool(payload.get("llm_root"))
        llm_model = payload.get("llm_model") or None
        show_logs = bool(payload.get("show_logs", True))
        show_metrics = bool(payload.get("show_metrics", True))
        script = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "ai_debugger", "main.py"))
        if not os.path.exists(script):
            print("Debugger not found. Expected: ai_debugger/main.py")
            return "ok"
        args = [sys.executable, script, "monitor", "--cmd", cmd]
        if show_logs:
        ...
        if ae:
            args.append("--ae")
        if llm_root:
            args.append("--llm-root-cause")
            if llm_model:
                args.extend(["--llm-model", llm_model])
        try:
            subprocess.run(args, check=False)
        except Exception as exc:
            debug_agent.log_error("debug-run", exc, intent=intent, payload=payload, command=cmd)
            print(f"Debug run failed: {exc}")
        return "ok"
    ...
    if intent == "workflow":
        ...
        if PARENTAL_AGENT.should_block_text(f"{target} {action_text}".strip()):
    ...
    if intent == "system-info":
        ...
        if not site:
            ...
            return "ok"
        if PARENTAL_AGENT.should_block_text(site) or PARENTAL_AGENT.should_block_url(site):
            print("This request is restricted.")
            return "ok"
        ...
        if not query:
            print("Query is required.")
            return "ok"
        if PARENTAL_AGENT.should_block_text(query):
            print("This request is restricted.")
            return "ok"
...
def _interactive():
    ...
    while True:
        ...
        if not user_input:
            continue
        block_reason = PARENTAL_AGENT.should_block_text(user_input)
        if block_reason:
            print("This request is restricted.")
            continue
        ...
        if user_input.lower().startswith("continuous "):
            try:
                ...
                def _loop():
                    while True:
                        for cmd in cmds:
                            if PARENTAL_AGENT.should_block_text(cmd):
                                print("This request is restricted.")
                                continue
