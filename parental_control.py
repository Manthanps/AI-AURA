...
EXTERNAL_LISTS = [
    ...
    "/Users/manthan/Library/Containers/net.whatsapp.WhatsApp/Data/tmp/documents/D11F2A3F-9341-460E-A68A-901EA7C3EFF3/fraud websites.txt",
    "/Users/manthan/Library/Containers/net.whatsapp.WhatsApp/Data/tmp/documents/F8BA3865-25C4-4CEF-B3C2-E0CB717410FE/banned.txt",
    "/Users/manthan/Library/Containers/net.whatsapp.WhatsApp/Data/tmp/documents/65B7C470-0AAF-4C8D-8BA9-DF3DA2822EAA/adult content.txt",
]
...
_CACHE_META: Optional[Tuple[float, str]] = None


def _external_candidates() -> List[str]:
    base = "/Users/manthan/Library/Containers/net.whatsapp.WhatsApp/Data/tmp/documents"
    names = {"fraud websites.txt", "banned.txt", "adult content.txt"}
    results: List[str] = []
    ...
    # add any explicit paths that still exist
    for p in EXTERNAL_LISTS:
        if os.path.exists(p) and p not in results:
            results.append(p)
    return results


def _rules_fingerprint(paths: List[str]) -> str:
    parts = []
...
def _load_rules() -> Tuple[List[str], List[str]]:
    ...
    fingerprint = _rules_fingerprint(external)
    now = time.time()
    if _CACHE is not None and _CACHE_META is not None:
        last_ts, last_fp = _CACHE_META
